      ...
      <header className="absolute left-0 right-0 flex items-center justify-between w-[97%] mx-auto px-12 top-9">
        ...
        <nav className="flex items-center gap-8">
          {navigationItems.map((item, index) => (
            <Link
              key={index}
              className={`h-7 text-xl text-center tracking-[-0.40px] [font-family:'Helvetica_Neue-Regular',Helvetica] font-normal text-[#212833] leading-7 whitespace-nowrap hover:opacity-70 transition-opacity ${item.isActive ? "text-[#004eff]" : "text-[#212833]"
                }`}
              to={item.href}
            >
              {item.label}
      ...
      <div className="absolute top-[207px] left-1/2 -translate-x-1/2 flex flex-col w-[900px] items-center justify-center gap-6">
        <div className="flex flex-col items-center justify-center gap-6">
          <div className="relative flex flex-col items-center gap-6">
            ...
            <img
              className="absolute top-[149px] left-[85px] w-[70px] h-[70px] object-cover rounded-full"
              alt="Ellipse"
